...
import { ToolRegistry, DEFAULT_DOMAINS, toMcpTool, toolNameFor, } from './core/registry.js';
import { AscApiError } from './core/errors.js';
import { ResourceStore } from './core/resources.js';
import { confirmWrite, buildWritePreview, resolveBodyRefs, } from './core/confirm.js';
...
import { METADATA_I18N_TOOLS, METADATA_I18N_TOOL_NAMES, METADATA_I18N_WRITE_TOOLS, executeMetadataI18nTool, } from './tools/metadata-i18n.js';
import { STOREKIT_TOOLS, STOREKIT_TOOL_NAMES, StoreKitService } from './storekit/index.js';
import { PRICING_TOOLS, PRICING_TOOL_NAMES, executePricingTool, buildPricingPreview, } from './tools/pricing.js';
...
import { STRONG_CONFIRM_LEVELS } from './core/risk.js';
import { CORE_OPERATIONS, manualToolsFor, deprecatedOperationsFor, operationsFor, CORE_MANUAL_TOOLS, profilesForOperation, registerCommand, removedProfileMessage, subProfileOwning, toolCountFor, TOKENS_PER_TOOL, } from './profiles.js';
import { stripApiNoise, redactPii, markUntrusted, capResponseSize, truncateText, DEFAULT_MAX_RESPONSE_CHARS, } from './core/shape.js';
...
export function servedToolCount(server) {
    return toolCounts.get(server)?.();
}
export function createServer(config, selection) {
    ...
    const missingToolHint = selection
        ? (op) => {
            ...
            if (ownSub) {
                ...
                return (`It belongs to this profile's "${ownSub.name}" sub-profile, which this server ` +
    ...
    const unloadedSubs = () => selection ? selection.profile.subProfiles.filter((s) => s.name && !loadedSubs.has(s)) : [];
    ...
    const profileReport = selection
        ? () => ({
            ...
            ...(unloadedSubs().length
                ? {
                    hint: `Some sub-profiles are not loaded. Call asc__load to add one for this ` +
    ...
    const manualTools = selection
        ...
        : undefined;
    const wantsFamily = (prefix) => manualTools ? [...manualTools].some((t) => t.startsWith(prefix)) : true;
     ...
     * safe.
     */
    const macroTools = [...PRICING_TOOLS, ...SCREENSHOT_TOOLS, ...ANALYTICS_TOOLS, ...PREFLIGHT_TOOLS, ...METADATA_TOOLS, ...ACCOUNT_TOOLS, ...METADATA_I18N_TOOLS, ...RELEASE_TOOLS].filter((t) => wantsFamily(`${t.name.split('__')[0]}__`) &&
        (!config.readOnly || t.annotations?.readOnlyHint === true));
    const macroOffered = (name) => macroTools.some((t) => t.name === name);
    /** The macros that declare an outputSchema, so their result can be sent structured. */
    const READ_MACRO_NAMES = new Set([...PRICING_TOOLS, ...SCREENSHOT_TOOLS, ...ANALYTICS_TOOLS, ...PREFLIGHT_TOOLS, ...METADATA_TOOLS, ...ACCOUNT_TOOLS, ...METADATA_I18N_TOOLS, ...RELEASE_TOOLS].filter((t) => t.outputSchema).map((t) => t.name));
    const server = new Server({ name: selection ? `asc-${selection.profile.name}` : 'app-store-connect-mcp', version: VERSION }, {
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (!found) {
            ...
            throw new Error(`No such resource: ${request.params.uri}. Only the ${resources.list().length} most ` +
    ...
    const isWriteTool = (name) => {
        ...
        if (macro)
            return macro.annotations?.readOnlyHint !== true;
    ...
    const loadTool = () => {
        ...
        return {
            name: 'asc__load',
            description: `Add one of this server's unloaded sub-profiles to the tool list for the rest of ` +
     ...
     */
    const profileOperations = () => selection
    ...
    const resolveProfileOperation = (raw) => {
        const wanted = String(raw).trim();
        const reachable = new Set(profileOperations());
        return OPERATIONS.find((op) => reachable.has(op.name) && (op.name === wanted || toolNameFor(op) === wanted));
    };
    /**
     ...
     */
    const PROXY_TOOLS = selection
        ? [
            {
                name: 'asc__describe',
                description: `Show the exact input schema of any tool this profile owns, including ones not ` +
            ...
            {
                name: 'asc__call',
                description: `Read through any tool this profile owns, even one that is not in your tool list — ` +
    ...
    const unreachableMessage = (raw) => {
        ...
        return (`"${raw}" is not part of this profile. ` +
            (homes.length
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (request) => {
        ...
        if (!prompt) {
            ...
            throw new Error(`Unknown prompt "${wanted}". This server offers: ${offered.join(', ') || 'none — its tools are in another profile'}.`);
        }
        ...
        return {
            ...
            messages: [
                { role: 'user', content: { type: 'text', text: prompt.body(args) } },
            ],
        };
    });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            if (name === 'asc__describe' && selection) {
                ...
                if (!op)
                    throw new Error(unreachableMessage(String(args.tool ?? '')));
            ...
            if (name === 'asc__call' && selection) {
                ...
                if (!op.readOnly) {
                    ...
                    throw new Error(`"${op.name}" changes data, and asc__call is read-only.\n` +
            ...
            if (config.confirmWrites !== 'off' && !config.dryRun && isWriteTool(name)) {
                ...
                // never going to show.
                const risk = PRICING_TOOL_NAMES.has(name)
                    ? 'revenue'
                    : RELEASE_TOOL_NAMES.has(name)
                        ...
                        : METADATA_I18N_WRITE_TOOLS.has(name)
                            ? // One call rewrites the store listing in every language it names.
                                ...
                                'public'
                            : (op?.risk ?? (name.includes('extend_renewal_date') ? 'revenue' : 'low'));
                if (config.confirmWrites === 'all' || STRONG_CONFIRM_LEVELS.has(risk)) {
                        ...
                        : op
                            ...
                            : macroOffered(name)
                                ? // Other write macros: no spec operation behind them, but their
                                    ...
                                    await buildWritePreview(name, { method: 'POST', path: `${name} (macro)`, risk: 'public' }, args, config.credentials.keyId)
                                : await buildWritePreview(name, 
                    ...
                    if (!decision.allowed) {
                        const text = decision.reason === 'no-elicitation'
            ...
            if (name === 'asc__load' && selection) {
                ...
                if (!sub) {
                    ...
                    throw new Error(selection.profile.subProfiles.some((s) => s.name === wanted)
                ...
                result = {
                    ...
                    nextStep: `These tools are live on this server now. If they are not yet in your tool list, ` +
                    ...
                    missingToolsHint: selection
                        ? (ops) => {
                            const own = ops
                                ...
                                .filter((n) => Boolean(n));
                            const homes = [
                                ...new Set(ops.flatMap((op) => profilesForOperation(op.name)).filter((n) => n !== selection.profile.name)),
                            ];
                            const lines = [];
                            if (own.length) {
                                lines.push(`Some are in sub-profiles this server did not load ` +
                                    `(${[...new Set(own)].join(', ')}) — restart it as ` +
                                    `\`${selection.profile.name}\` to load all of them.`);
                            }
                            if (homes.length) {
                                lines.push(`The rest live on sibling MCP servers. Register the ones you need — ` +
                ...
                return (await executeMetadataI18nTool(name, args, { http }));
            }
                ...
                if (result?.dryRun === true && args.body !== undefined) {
                    const labels = await resolveBodyRefs(http, args.body?.data);
                    if (labels.size) {
                        result.humanReadable = Object.fromEntries(labels);
                    }
                }
            ...
            const text = truncateText(JSON.stringify(result ?? { ok: true }, null, 2), maxResponseChars());
            ...
            return {
                content: [{ type: 'text', text: formatError(err, name) }],
                isError: true,
            };
        }
    });
 ...
 */
export function createRemovedProfileServer(name) {
    const message = removedProfileMessage(name);
    if (!message)
        throw new Error(`"${name}" is not a removed profile.`);
    const server = new Server({ name: `asc-${name}`, version: VERSION }, { capabilities: { tools: {} } });
    const tool = {
        name: 'asc__profile_removed',
        description: `This MCP server is configured for the App Store Connect profile "${name}", which was ` +
 ...
 */
export function formatError(err, toolName) {
    ...
    if (!(err instanceof AscApiError) || !err.status) {
        return `${toolName} failed: ${err instanceof AscApiError ? err.summary : err.message}`;
    }
